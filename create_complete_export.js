// Complete Database Export with Schema + Data for Full Import
...
async function createCompleteExport() {
    console.log('Creating complete database export with schema + data...');
    
    let exportSQL = '-- Complete Database Export with Schema + Data\n';
    exportSQL += `-- Generated: ${new Date().toISOString()}\n`;
    exportSQL += `-- Project: ${SUPABASE_URL}\n`;
    exportSQL += `-- Purpose: Full import to another Supabase instance\n\n`;
    
    // Table schemas with proper PostgreSQL types
    const tableSchemas = {
    ...
    // Now add data for all accessible tables
    ...
    // Save to file
    const fs = require('fs');
    fs.writeFileSync('database_complete_schema_data.sql', exportSQL);
    
    console.log('Complete database export finished! Saved to database_complete_schema_data.sql');
    console.log(`Exported ${tables.length} tables with schema + data`);
}
