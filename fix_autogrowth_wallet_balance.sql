-- Fix the autogrowth function to properly update wallet_balances total
...
BEGIN
    ...
    LOOP
        ...
        -- Update wallet balance if exists (FIXED: update both available and total)
        UPDATE wallet_balances 
        SET 
            available = new_balance - user_balance.locked_amount,  -- Keep locked amount separate
            total = new_balance,  -- Update total to include growth
            updated_at = NOW()
        WHERE user_id = user_balance.user_id 
            AND currency = user_balance.currency;
...
GRANT EXECUTE ON FUNCTION calculate_daily_autogrowth() TO authenticated;
